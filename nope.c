...
void accept_connection(FdData *fds, int listenfd, char *remote_ip, int *fdmax, fd_set *master)
{
	socklen_t addrlen;
	...
	while (1) {
		...
		for (e = 0; e < n; e++) {
			...
			else
			{
				...
				{
					i=events[e].data.fd;
					nbytes = read (i, fdDataList[i].readBuffer+fdDataList[i].readBufferIdx, MAX_REQUEST_SIZE - fdDataList[i].readBufferLen);
					if (nbytes == -1)
					{
