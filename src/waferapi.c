...
long resPrintf(Response * response, const char *format, ...)
{
    dbgprintf("Flags %d API Flags% d response.status %d\n", response->flags,
              response->apiFlags, response->status);
               ...
               const char *displayFilename, const char *type)
{
    ...
    if (pFile == NULL) {
        ...
        struct tm lastmod;
        gmtime_r(&stat_buf.st_mtime, &lastmod); //got last modification time

        if (request) {          // some very optional functionality that tries to avoid 
            for (unsigned int i = 0; i < request->headersLen; i++) {

                if ((strncmp(request->headers[i], "If-Modified-Since:", 18) == 0)
                    && (strlen(request->headers[i]) > 19)) {

                    struct tm modsince;
                    strptime(request->headers[i] + 18, HTTP_TIME_FORMAT, &modsince);

                    time_t modsince_t = timegm(&modsince);
                    if ((stat_buf.st_mtime) <= modsince_t) {

                        resPrintf(response, "HTTP/1.1 304 Not Modified\r\n");
                        fclose(pFile);
                        return;
                    }
                }
        ...
        char lastmodtime[128];
        strftime(lastmodtime, 128, "%a, %d %b %Y %H:%M:%S GMT", &lastmod);

        resPrintf(response, "Last-Modified: %s\r\n", lastmodtime);
...
char *getQueryPath(Request * request)
{
    ...
    u_int i;
    for (i = 0;
         i < strlen(request->reqStr) && (queryPath[i] != '?') && (queryPath[i] != '\0');
         i++) {
    }
